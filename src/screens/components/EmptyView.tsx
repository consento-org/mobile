...
interface IEmptyViewProto {
  ...
  addButton?: Link<IAddButton, IButtonLabel>
}

export function EmptyView ({ prototype: proto, onAdd }: { prototype: IEmptyViewProto, onAdd?: () => void }): JSX.Element {
  return <View style={{ flexGrow: 1, display: 'flex' }}>
    <ScrollView
      centerContent
      style={{ backgroundColor: proto.backgroundColor }}
      contentContainerStyle={{ flexDirection: 'row', display: 'flex', flexGrow: 1 }}>
    ...
    {proto.addButton !== undefined
      ? <View style={{ position: 'absolute', bottom: 0, width: '100%', height: proto.bottomArea.place.height, display: 'flex', alignContent: 'center', justifyContent: 'center', alignItems: 'center' }}>
        ...
        <ConsentoButton title={proto.addButton.text.label} style={{ width: 200, height: 36 }} light onPress={onAdd} />
      </View>
      : null}
  </View>
}
